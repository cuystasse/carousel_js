...
function dateSort2($a, $b) {
	if ($a[0]['releaseDate'] > $b[0]['releaseDate']) {
		return 1;
	}
	return -1;
}
...
// drop sets
$all_sets = file_get_contents("https://api.magicthegathering.io/v1/sets");
$all_sets = json_decode($all_sets, true)['sets'];
$sets = [];
foreach ($all_sets as $set) {
	if (isset($set['block'])) {
		// avoid set without lands
		// $set_code = strtolower($set['code']);
		// $landUrlCount = 'https://api.magicthegathering.io/v1/cards?name=plains&set='.$set_code.'&supertypes=Basic';
		// $lands = get_headers($landUrlCount, 1);
		// $count_lands = $lands['Count'];
		// if ($count_lands < 1) {
		// 	continue;
		// }

		$sets[$set['block']][] = $set;
		uasort($sets[$set['block']], 'dateSort');
	}
}

uasort($sets, 'dateSort2');
//var_dump($sets);die();
...
<body>
	<h1>MTG Land Carousel with JQuery</h1>
	<div class="subtitle">
		<span></span>
		<h2 id='active-set-h2'></h2>
		<span></span>
	</div>

	<div id="carousel" data-current="0" data-max="2">
		<div class="filter filter-left">
			<span id="left" class="fa fa-chevron-circle-left"></span>
		</div>
		...
		<div class="filter filter-right">
			<span id="right" class="fa fa-chevron-circle-right"></span>
		</div>
	</div>
	...
	<div id="lands">
		...
		<!-- B -->
		<div data-land="swamp" class="land black">
		...
		<!-- R -->
		<div data-land="mountain" class="land red">
		...
		<!-- G -->
		<div data-land="forest" class="land green">
	...
	<div class="choose-set">
		...
		<select>
			<?php foreach ($sets as $block => $exts) :?>
				<optgroup label="<?= $block ?>">
					<?php foreach ($exts as $ext) :?>
						<option value="<?= $ext['code'] ?>"> <?= $ext['name'] ?></option>
					<?php endforeach ?>
				</optgroup>
			<?php endforeach ?>
		</select>
